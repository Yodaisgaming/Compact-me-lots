...
function segments(data) {
  const state = { pending: '', pasting: false };
  const out = streamSegments(data, state).map((item) => item.data);
  if (state.pending) out.push(state.pending);
  return out;
}
...
function decodeWin32(seg) {
  ...
  if (uc >= 32 || uc === 9) return { kind: 'text', chars: rc };
...
class InputTracker {
  ...
  note(data, now) {
    ...
    if (!parts.length && data && this.segmentState.pending) this.seq++;
    for (const item of parts) {
      ...
      switch (kind) {
        case 'submit':
          ...
          break;
        case 'backspace':
          this.pendingChars = Math.max(0, this.pendingChars - (w ? w.chars : 1));
          this.seq++;
          break;
        case 'text':
...
module.exports = { classify, segments, InputTracker };
